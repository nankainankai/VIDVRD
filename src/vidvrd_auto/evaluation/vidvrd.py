...
"""Trajectory-aligned ImageNet-VidVRD evaluation."""
...
def _confidence(item: Dict[str, Any]) -> float:
    try:
        return float(item.get("confidence", 1.0) or 0.0)
    except (TypeError, ValueError):
        return 0.0


def _tiou(left: Dict[str, Any], right: Dict[str, Any]) -> float:
    left_start, left_end = int(left.get("start_frame", 0)), int(left.get("end_frame", 0))
    right_start, right_end = int(right.get("start_frame", 0)), int(right.get("end_frame", 0))
...
def run_vidvrd_eval(
    *,
    ...
    prediction_videos = set(pred_tracks)
    ...
    metrics: Dict[str, Any] = {
        ...
        "recall_at_50": _recall_at_k(predictions, gold, mappings, relation_tiou_threshold, 50),
        "recall_at_100": _recall_at_k(predictions, gold, mappings, relation_tiou_threshold, 100),
        "tagging_precision_at_1": _tagging_precision_at_k(predictions, gold, mappings, 1),
        "tagging_precision_at_5": _tagging_precision_at_k(predictions, gold, mappings, 5),
        "tagging_precision_at_10": _tagging_precision_at_k(predictions, gold, mappings, 10),
    ...
    lines = [
        ...
        f"- Tracks: {matched_tracks}/{gold_track_count} matched, mean vIoU={metrics['tracks']['mean_viou']:.4f}",
        "",
        "## Overall",
        "",
        f"TP={overall['tp']}, FP={overall['fp']}, FN={overall['fn']}, Precision={overall['precision']:.4f}, Recall={overall['recall']:.4f}, F1={overall['f1']:.4f}",
        f"Micro AP={overall['ap']:.4f}, predicate mAP={mean_ap:.4f}",
        f"Recall@50={metrics['recall_at_50']:.4f}, Recall@100={metrics['recall_at_100']:.4f}",
        f"Tagging P@1={metrics['tagging_precision_at_1']:.4f}, P@5={metrics['tagging_precision_at_5']:.4f}, P@10={metrics['tagging_precision_at_10']:.4f}",
        "",
        "## Base / novel",
        "",
