...
def keyframe_screen_prompt(frame_results: List[Dict[str, Any]], *, has_images: bool = False) -> str:
    visual = "附图是按时间顺序排列的关键帧拼图（已画检测框）。请结合画面与统计判断。\n" if has_images else ""
    return (
        "你是视频关系标注质检助手。请判断视频是否值得继续标注。\n"
        f"{visual}"
...
def track_qc_prompt(risk_items: List[Dict[str, Any]], *, has_images: bool = False) -> str:
    visual = "附图是存在风险的抽帧拼图（已画轨迹框与 ID）。请结合画面判断。\n" if has_images else ""
    return (
...
def global_relation_prompt(relations: List[Dict[str, Any]], *, has_images: bool = False) -> str:
    visual = "附图是全片均匀抽帧拼图，用于辅助判断跨片段动态关系。\n" if has_images else ""
    return (
        "你是视频级关系复核助手。请根据片段候选关系判断是否存在跨片段持续关系和动态关系。\n"
        f"{visual}"
...
def relation_verify_prompt(relations: List[Dict[str, Any]], issues: List[Dict[str, Any]], *, has_images: bool = False) -> str:
    visual = "附图是全片均匀抽帧，用于辅助判断关系冲突与低置信度项。\n" if has_images else ""
    return (
